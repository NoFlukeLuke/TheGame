...
## Tutorial mode (`js/tutorial.js` + `css/tutorial.css`, r123+)

A guided first run, listed **first** in the mode-select carousel. `MODES.tutorial` has `actStructure: true`, so it is mechanically **identical to Classic** — the real round → payout → reward grid → shop loop, not a sandbox. The `tutorial: true` flag (checked via `tutorialActive()`) only turns on the rigging and the coach-marks. 15 steps, ~2 minutes, skippable at any point; when it ends the run just carries on as a normal Classic run.

- **Polled state machine, not events.** Each step declares `when` (hold the step back until true) and `until` (auto-advance when true) as predicates over existing globals — `selected.length`, `handsPlayed`, `goalReachedThisRound`, `document.body.classList.contains('reward-active')`, and so on. One rAF loop evaluates them. **This is why the tutorial needs almost no hooks in engine code** — the only edits outside `js/tutorial.js` are the `MODES` entry, one call at the end of `startGame()`, the auto-submit guard in `input.js`, and the forced Shop destination in `reward-grid.js`. Adding/reordering steps means editing `TUTORIAL_STEPS` and nothing else.
- **The layer lives OUTSIDE `#stage`** (appended to `<body>`), same reason as the dev panel: inside the cabinet it would inherit the CSS `zoom`. All positioning uses raw `getBoundingClientRect()` viewport px.
- **Spotlight** = the `box-shadow: 0 0 0 100vmax` spread trick on a small `#tut-spot` rect (as in `css/dev-overlays.css`), so the "hole" needs no mask. Box-shadow isn't hit-testable, so clicks pass through by default; four transparent `.tut-shutter` rects around the hole are what actually block input, and only while the step is `.gated`. A gated step therefore exposes exactly one control (used to force "select three cards", then "press PLAY").
- **Anchors must be the real positioned widgets.** In landscape, `#score-panel`, `#hand-preview-area` and `#action-col` are `display: contents` — zero rect. Anchor `#score-center` / `#score-left` / `#score-subboxes` / `#selected-cards` / `#btn-discard` instead. `tutEl(...sels)` picks the first *visible* match (so one step covers `#vclock` and its portrait `#clock-area` fallback); `tutEls(...)` unions several into one hole (SCORE + GOAL).
- **Three bubble modes:** anchored (spotlight + arrow), `side:'center'` (flat veil, centred card — the intro/outro), and `side:'float'` (no spotlight, **no dim**, bubble parked bottom-left). Float exists for the "now clear the round" step: the player has to actually play, so the board can't sit behind a veil.
- **Rigging** (`tutorialBeginRun`, called last in `startGame`): `roundGoal` → `TUTORIAL_GOAL` (500, ~3 hands), `coins` → 14 so the shop is affordable, and `tutorialStackBoard()` puts three Kings in an L at `[1,1] [1,2] [2,1]`. Kings are **moved** (swapped with whatever occupied the cell, from the grid or the draw pile), never minted, so the deck audit stays intact.
- **Gotchas the steps work around:** the hand preview (`#selected-cards`) is deliberately **empty until a hand is submitted** (r99) — it's a stage for the scoring dance, not a live selection readout, so the "read your hand" step points at the PIPS/MULT chips instead. Auto-submit (2s) would fire the hand out from under the early steps, so `tutorialHoldsAutoSubmit()` suppresses it on steps flagged `noAutoPlay`. The scoring dance runs ~6.5s, so the step after PLAY waits on `tutIdle()` and stays hidden meanwhile rather than dimming the animation.
- **Anti-stall:** a step whose `when` never flips shows anyway after `whenTimeoutMs` (default 20s). Without it a stalled predicate would leave the tutorial silently dead with the player none the wiser.
- Clock handling: `tutorialHoldClock()` only ever releases a pause **it** took (`_tutClockHeld`), so the reward grid and shop keep ownership of `gameTimerPaused` during their own steps.
