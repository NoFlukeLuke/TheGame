...
## Flow mode (`js/flow-mode.js`, r164) — Survival with no round clock
...
- **Two traps this encodes.** (1) **First Wind** measured its grace window as `ROUND_DURATION - roundSeconds`; Flow's clock starts at 300 against a `ROUND_DURATION` of 180, so that is **negative** and decay would be held off for the first ~165s of every session — it measures from `roundStartSeconds` in Flow instead. (2) **Carry Time** banks the round's unused seconds; Flow's clock isn't reset by a level-up, so it would pay out the same seconds every level and is skipped.
