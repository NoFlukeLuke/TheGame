...
- **TBD — bosses (stage 2).** Owner wants a boss every 8 clears and no challenges. Challenges already never fire (reward-grid/act-driven). Leftover time is **already banked** (`survivalBossTimeBank`, capped 180) to feed the boss clock, but the boss is **not yet triggered** — wiring `triggerBoss` + a non-reward-grid post-boss reward into survival is the open follow-up.
