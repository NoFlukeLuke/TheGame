...
- **Build stamp:** bump the `BUILD` constant at the top of **`js/menu.js`** (currently `'2026-08-20 · r148 · Orientation board is audited + re-dealt; swap and discard are now taught hands-on'`) on every commit. It shows in the menu footer + dev panel so the owner can confirm the cache is fresh. Increment the `rN` each commit.
