...
- **Build stamp:** bump the `BUILD` constant at the top of **`js/menu.js`** (currently `'2026-08-04 · r123 · Match-3 level-up: on-grid pick-of-3 (centre tiles)'`) on every commit. It shows in the menu footer + dev panel so the owner can confirm the cache is fresh. Increment the `rN` each commit.
...
- **Between-rounds level-up** (r122–r123): match-3 has its OWN flow (`showMatch3LevelUpScreen` in `js/match3.js`), delegated from the top of `showLevelUpScreen`. It deals a fresh full board, settles it, then drops **three Trick options onto the centre tiles** (`match3CenterCells`) as tappable on-grid cards — a reward screen shaped like Normal mode's on-grid pick but only three tiles, not a full board. Tap-to-preview / tap-again-to-confirm reuses the shared `onTrickTap` path, intercepted for match-3 to call `match3ConfirmPick`, which sends the chosen Trick to the **side tray** (`trickTray.push` + `selectTrick`) and reverts all three option tiles to normal cards — nothing stays on the board for play. `match3PickActive` gates the phase: `match3Resolve`, `onCardTap` and `doDiscard` all bail while it's true, and a `LEVEL_UP_DURATION` timer auto-picks the first option. The legacy else-branch in `showLevelUpScreen` (the bug) placed Tricks for keeps and sized the pick from empty grid cells, which is why match-3 was offering just one.
