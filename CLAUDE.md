...
Six changes, all in service of "what the HUD says should be where you'd look for it".
...
- **SWAP is a candy cap now**, cyan — the one accent PLAY (yellow) and DISCARD (coral) don't use. It was a dark inset plaque with cyan text, which read as scenery rather than a resource you spend. Every rule is `:not(.reward-skip)`: the reward grid repurposes this same element into a blue SKIP button and that skin must keep winning.
