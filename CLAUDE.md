...
Three cards of one suit in sequence are a Run of 3 **and** a Flush of 3. `detectHand` picked one and threw the other away, which is why building the harder shape felt like it paid nothing extra. Now every hand the cards satisfy is a **layer**: each layer adds its printed base pips and base mult, each earns its own Natural Scaling, and **every card in a layered hand scores one extra time**. Measured on a live board: a same-suit Run of 3 went 129 -> 380.

- **THE RULE IS ONE LAYER PER FAMILY** (set / run / flush), best-worth wins. Without it a Full House would also claim Three of a Kind, Two Pair and Pair - that is one shape read four ways, not four shapes, and it would pay four times for the same five cards. **Straight Flush occupies both families on its own**, which is correct: it already IS the run and the flush, so nothing layers onto it.
- **`handLayersFor(primary, cells)` is called INSIDE `calcScore`, not passed in by its six callers.** That is deliberate: the live PIPS/MULT chips, the scoring dance, `findBestHand`'s comparison and the committed score then cannot disagree about what was played. `captureRoundContrib` counts the layers too, or the payout breakdown reports less base than the scorer used.
- **`detectHand` now returns the BEST-PAYING reading**, not the first hit in a fixed priority list. The old list had one special case bolted on ("flush of 3 unless the run of 3 is worth more") and its scope was too narrow: three same-suit cards of one rank were named Flush of 3 (30) over Three of a Kind (105), and four same-suit cards holding two pairs were named Flush of 4 (60) over Two Pair (90). **`handMatchesFor`'s order is still load-bearing** - it is the old priority list, and it breaks ties, which is the normal case in the no-pips scoring models where worth is the mult alone.
- **`_handShape(cells)` is the one place the cards are read.** `detectHand` and `handMatchesFor` must never disagree about what the cards ARE, so both call it.
- **In Classic almost nothing layers today, and that is a live design question.** `flush3`/`flush4` are only seeded at `suitCount >= 6`, so in a 4-suit deck the only overlaps reachable are through the **Short Suit** knack (which layering makes far stronger - it turns every same-suit run into a double hand) or through duplicated cards from the shop. Turning short flushes on in Classic is a balance decision, deliberately NOT taken here.
- Dev toggle: **Score -> Layered hands** (`layeredHandsEnabled`, persisted, default on).
- **Not yet done: hands of 6-7 cards.** `findBestHand` still caps subsets at 5. Layering answers "one shape, several readings"; a 7-card selection holding a Run of 4 *and* a separate Set of 3 is a different mechanism - partitioning a selection into two disjoint hands - and the hand types that are legal at that size are still an open question.
