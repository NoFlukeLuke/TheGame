...
- **Tempo** (Knack) — **sets** the swap and discard limits to 2, then drips 1 back every 15s alternating swap → discard → swap (the round-timer tick in `round-timers.js`; alternation advances even when that stock is full, so the rhythm never stalls). Because it *sets* rather than adds, it deliberately overrides limit-break upgrades, Swap Shop / Harvest, and makes Pack Rat / Collector carry-over moot — enforced in two places: `computeRoundResources()` returns a flat 2/2, and `syncKnackLimits()` (in `limits.js`, called from `updateKnackList()` so every acquisition path hits it) rewrites `limits.*.current` and clamps the live stock the moment it's acquired. Steady Hand / Hoarder still work — they bypass spending the stock entirely, which makes them a strong (but time-expensive) Tempo pairing.
...
## Spectrum mode — the numeric colour deck (`MODES.spectrum`, r160–r161)

Classic's 3-Act game on a deck with **no suits and no court cards**: seven **colours** (🔴 🟡 🔵 🟢 🟣 🟠 ⚫) and the values **0–11 plus a lone 15 and a lone 20** — 7 × 14 = **98 cards**, plus the four payout fixtures below. Everything else (rounds, reward grid, events, Mart, bosses, Tricks, Sleights, Knacks, Focus) is Classic's, untouched.
