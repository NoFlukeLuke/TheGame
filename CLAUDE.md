...
`siphonMultX` / `growthSpurtCapPenalty` / `growthSpurtMaxedThisRound` are declared in `combos-aim.js` and reset on new game (`game-control.js`). **Sleight discard-on-use note:** only active-tap sleights leave the grid after use; **passive** (Whetstone/Entourage/Lighthouse/Fight the Power/Slow Burn), **aim** (Reflect/Soul Mirror) and **wildcard** sleights don't — they work by *sitting on the grid* or leave by being played. The existing double-tap sleights (Amplifier/Snooze/Piggy Bank/Magnet/Stopwatch) still use the once-per-round lock; only Capacitor/Siphon discard-on-use so far. See `FOCUS_IDEAS.md` for the larger focus-mode backlog.
