...
- **`#score-center` has two click handlers.** r172's `bindBossBriefReopen()` makes the alarm-state SCORE/GOAL chips the handle that reopens the boss briefing; `js/hand-log.js` binds click on the same element to toggle the hand log. `stopPropagation` does **not** separate them — they are sibling listeners on one node, not ancestor and descendant. The hand log yields the CLICK while `bossActive` (the brief is the more urgent of the two) and keeps hover and long-press, neither of which the brief binds.
