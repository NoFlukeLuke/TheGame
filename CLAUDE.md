...
- **No stacked deck — it finds a hand instead.** `tutorialFindTeachingHand()` grows connected shapes over the dealt board and highlights a real one, preferring hands where *every* highlighted card scores (a pair plus a dead third card teaches the wrong thing).
