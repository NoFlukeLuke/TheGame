...
- **Build stamp:** bump the `BUILD` constant at the top of **`js/menu.js`** (currently `'2026-08-04 · r122 · Match-3 level-up: tray pick-of-3 + win finale'`) on every commit. It shows in the menu footer + dev panel so the owner can confirm the cache is fresh. Increment the `rN` each commit.
...
- **Between-rounds level-up** (r122): match-3 has its OWN flow (`showMatch3LevelUpScreen` in `js/match3.js`), delegated from the top of `showLevelUpScreen`. It deals a fresh full board (no Tricks on it), offers a genuine **pick-of-three** via the existing `#trick-choice-overlay`, and the chosen Trick goes to the **side tray** (`confirmMatch3TrickSelection` → `trickTray.push` + `selectTrick`), never the grid. The legacy else-branch in `showLevelUpScreen` placed Tricks on the board and sized the pick from empty grid cells (which is why match-3 was offering just one). `showTrickChoiceOverlay` / the timer-expiry / the skip button all branch on `match3Active()` to route to the match-3 confirm.
