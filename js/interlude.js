...
async function showLevelUpScreen_fallOnly() {
  ...
  if (focusNodes > 0) {
    ...
    const charges = Math.floor(total / FOCUS_THRESHOLD);
    const segNodes = total - charges * FOCUS_THRESHOLD;

    // Dots first
    const litDots = [];
    for (let i = 0; i < Math.min(charges, 3); i++) {
      const dot = document.getElementById(`focus-dot-${i}`);
      if (dot && dot.classList.contains('lit')) litDots.push(dot);
    }
    litDots.forEach((d, i) => spawnFocusFallClone(d, { delay: i * 40 }));

    // Nodes after dots
    const DOT_LEAD = litDots.length > 0 ? 100 : 0;
    const filledNodes = [];
    for (let i = 0; i < segNodes; i++) {
      const n = focusNodeEls[i];
      if (n) filledNodes.push(n);
    }
    filledNodes.forEach((n, i) => spawnFocusFallClone(n, { delay: DOT_LEAD + i * 30 }));
